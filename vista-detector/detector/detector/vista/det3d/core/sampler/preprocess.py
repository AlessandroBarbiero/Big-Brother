...
#             boxes[i, -1] += rot_transform[i]
#             if len(boxes[i])>7:
#                 boxes[i, 6:8] = box_np_ops.rotation_points_single_angle(
#                     np.concatenate([boxes[i:i+1, 6:8], np.zeros((1, 1))], -1),
#                     rot_transform[i],
#                     axis=2,
#                 )[0, :2]

def box3d_transform_(boxes, loc_transform, rot_transform, valid_mask):
    boxes[valid_mask, :3] += loc_transform[valid_mask]
    boxes[valid_mask, -1] += rot_transform[valid_mask]
    if boxes.shape[1]>7:
        boxes[valid_mask, 6:8] = box_np_ops.rotation_2d(np.expand_dims(boxes[valid_mask, 6:8],1),rot_transform[valid_mask]).reshape(boxes[valid_mask, 6:8].shape)
