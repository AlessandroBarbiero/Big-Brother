...
class Bev:
    ...
    def computeSize(self, outView, outImageSize):
        ...
        if not np.any(np.isnan(outImageSize)):
            self.scaleXY = np.flip((np.array(outImageSize) - 1) / worldHW)
            self.outSize = outImageSize
        else:
            notSpecifSize_i = np.argwhere(np.isnan(outImageSize))[0][0]
            specifSize_i = np.argwhere(np.logical_not(np.isnan(outImageSize)))[0][0]
    ...
    def projectBevPointsToWorldGroundPlane(self, bev_points):
        world_points_z0_h = np.linalg.inv(self.camera.P[:, [0, 1, 3]]).dot(
