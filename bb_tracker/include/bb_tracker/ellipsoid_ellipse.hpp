 ...
 * @return       A 4-element vector representing ellipse variables:
...
Eigen::Vector4f ellipseFromEllipsoidv1(Eigen::Matrix<float, 1, 8> state, TRANSFORMATION vMat, PROJ_MATRIX P){
  ...
  float etheta;
  etheta = atan2(e_rotmat(1, 0), e_rotmat(0, 0));  // TODO: Not used yet 
...
Eigen::Vector4f ellipseFromEllipsoidv2(Eigen::Matrix<float, 1, 8> state, TRANSFORMATION vMat, PROJ_MATRIX P){
    // TODO: add theta computed from eigenvectors, the problem is that the axes are always ordered by length, this is not good, the ellipses always appear vertical
    Eigen::Vector4f result;
    // State
    float x = state(0),
    y =       state(1),
